...
def run_grounding(
    ...
    return {
        ...
        "note": (
            "GroundingDINO here is stock, not fine-tuned on overhead imagery. "
            "These numbers are the pre-adaptation baseline."
        ),
    }
...
def main() -> int:
    ...
    ap.add_argument(
        "--task",
        choices=["vqa", "caption", "change", "grounding", "all"],
        default="all",
    )
    ...
    tasks = ("vqa", "caption", "change", "grounding") if args.task == "all" else (args.task,)
