...
def load_task_data(
    ...
    if task == "vqa":
        ...
        # BigEarthNet-MM: real co-registered Sentinel-1 SAR + Sentinel-2 optical.
        for split, sink in (("train", train), ("val", val)):
            for row in read_jsonl(data_dir / f"{split}.jsonl"):
                if keep(row["answer"]):
                    sink.append(
        ...
        # PaliGemma's native detect task. The target is location tokens, which
        # depend on the letterboxed canvas, so they are built in the collate
        # function once the image's real size is known.
        for split, sink in (("train", "grounding_train"), ("val", "grounding_val")):
            for row in read_jsonl(data_dir / f"{sink}.jsonl"):
                phrase = str(row.get("phrase", "")).strip()
                bbox = row.get("bbox")
                if not phrase or not isinstance(bbox, list) or len(bbox) != 4:
                    continue
                target = train if split == "train" else val
                target.append(
                    {
                        "kind": "grounding",
                        "images": [row["image"]],
                        "question": phrase,
                        "answer": "",              # filled in at collate time
                        "bbox": [float(c) for c in bbox],
                        "bbox_convention": row.get("bbox_convention", "absolute_pixels"),
                        "bbox_divisor": row.get("bbox_divisor"),
                        "key": row.get("image_id", row["image"]),
                    }
                )
    else:
...
def build_prompt(record: Dict[str, Any]) -> str:
    ...
    if kind == "fusion":
        return build_fusion_prompt(record["question"])
    if kind == "grounding":
        return build_detect_prompt([record["question"]])
    raise ValueError(f"Unknown record kind {kind!r}")


def render_image(record: Dict[str, Any], size: int) -> Tuple[Image.Image, Tuple[int, int]]:
    """
    Build the exact pixel input the model will see, plus the source image's
    original size.

    The original size is needed for the grounding task: boxes are annotated in
    the source image's pixel space, but the model sees a letterboxed square
    canvas. Encoding untransformed coordinates would teach the model boxes that
    do not sit on the objects.
    ...
    paths = record["images"]
    if len(paths) == 1:
        raw = Image.open(paths[0]).convert("RGB")
        return make_single_image(raw, size=size), raw.size
    img1 = Image.open(paths[0]).convert("RGB")
    img2 = Image.open(paths[1]).convert("RGB")
    return make_pair_composite(img1, img2, size=size, layout="horizontal"), img1.size


def absolute_bbox(record: Dict[str, Any], orig_size: Tuple[int, int]) -> List[float]:
    """Convert an annotated box into absolute source-image pixels."""
    box = list(record["bbox"])
    divisor = record.get("bbox_divisor")
    if divisor:
        w, h = orig_size
        box = [
            box[0] / divisor * w,
...
def resolve_answer(record: Dict[str, Any], orig_size: Tuple[int, int], size: int) -> str:
    """The supervision target. Grounding builds location tokens on the fly."""
    if record["kind"] != "grounding":
        return record["answer"]
    abs_box = absolute_bbox(record, orig_size)
    canvas_box = box_to_canvas(abs_box, orig_size, size)
    return build_detect_suffix([canvas_box], [record["question"]], canvas_size=size)
...
def make_collate_fn(processor, image_size: int, max_length: int):
    def collate(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        rendered = [render_image(ex, image_size) for ex in examples]
        images = [r[0] for r in rendered]
        answers = [
            resolve_answer(ex, r[1], image_size) for ex, r in zip(examples, rendered)
        ]
...
def evaluate(
    ...
    golds: List[str] = []
    ious: List[float] = []
    is_grounding = subset[0]["kind"] == "grounding"

    for i, ex in enumerate(subset):
        image, orig_size = render_image(ex, image_size)
        ...
        # skip_special_tokens would strip the <locNNNN> tokens the detection
        # task is entirely made of, so keep them for grounding.
        pred = processor.decode(
            out[0][prompt_len:], skip_special_tokens=not is_grounding
        ).strip()
        gold = resolve_answer(ex, orig_size, image_size)
        preds.append(pred)
        golds.append(gold)

        if is_grounding:
            gold_box = box_to_canvas(absolute_bbox(ex, orig_size), orig_size, image_size)
            decoded = decode_loc_tokens(pred, canvas_size=image_size)
            ious.append(max((_iou(b, gold_box) for b, _ in decoded), default=0.0))
    ...
    if ious:
        n = len(ious)
        metrics["mean_iou"] = round(sum(ious) / n, 4)
        metrics["acc@0.5"] = round(sum(1 for v in ious if v >= 0.5) / n, 4)
        metrics["acc@0.25"] = round(sum(1 for v in ious if v >= 0.25) / n, 4)
        metrics["no_box_decoded_rate"] = round(sum(1 for v in ious if v == 0.0) / n, 4)

    # The degeneracy check that would have caught the previous adapter.
    # Detection strings are near-unique by construction, so the distinct-ratio
    # test is meaningless there; a collapsed detector shows up as IoU instead.
    if ious:
        metrics["collapsed"] = bool(metrics["mean_iou"] < 0.10)
    else:
...
def main() -> int:
    ...
    ap.add_argument("--task", choices=["vqa", "change", "fusion", "grounding"], required=True)
    ...
    if args.task == "grounding":
        # Detection targets are coordinates, not a closed answer set; measure
        # diversity by referring phrase instead.
        gold_variety = len({r["question"].strip().lower() for r in train_records})
        print(f"Distinct referring phrases: {gold_variety:,}")
    else:
        gold_variety = len({r["answer"].strip().lower() for r in train_records})
        print(f"Distinct train answers  : {gold_variety:,}")
    if gold_variety < 20:
        raise SystemExit(
            f"Only {gold_variety} distinct targets in the training set. "
