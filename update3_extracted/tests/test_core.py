...
# ---------------------------------------------------------------------------
# location tokens and box geometry
# ---------------------------------------------------------------------------


def test_box_canvas_roundtrip_is_lossless():
    orig = (500, 250)
    box = [100.0, 60.0, 300.0, 200.0]
    canvas = box_to_canvas(box, orig, 224)
    back = box_from_canvas(canvas, orig, 224)
    assert back == pytest.approx(box, abs=1.0)


def test_box_to_canvas_respects_the_letterbox_offset():
    """A wide image is centred vertically, so y gains the letterbox offset."""
    orig = (400, 200)
    scale, dx, dy, _, _ = fit_transform(orig, 224)
    assert dx == 0 and dy > 0
    canvas = box_to_canvas([0.0, 0.0, 400.0, 200.0], orig, 224)
    assert canvas[0] == pytest.approx(0.0, abs=0.5)
    assert canvas[1] == pytest.approx(dy, abs=0.5)


def test_loc_token_roundtrip():
    box = [20.0, 40.0, 180.0, 200.0]
    encoded = encode_loc_tokens(box, canvas_size=224)
    decoded = decode_loc_tokens(encoded + " water body", canvas_size=224)
    assert len(decoded) == 1
    got, label = decoded[0]
    assert got == pytest.approx(box, abs=0.5)
    assert label == "water body"


def test_loc_tokens_are_ordered_y_x_y_x():
    """PaliGemma emits y_min, x_min, y_max, x_max - not xyxy."""
    encoded = encode_loc_tokens([0.0, 224.0, 0.0, 224.0], canvas_size=224)
    values = [int(v) for v in re.findall(r"<loc(\d{4})>", encoded)]
    assert values == [1023, 0, 1023, 0]


def test_loc_tokens_normalise_inverted_boxes():
    a = encode_loc_tokens([180.0, 200.0, 20.0, 40.0], canvas_size=224)
    b = encode_loc_tokens([20.0, 40.0, 180.0, 200.0], canvas_size=224)
    assert a == b


def test_decode_handles_multiple_detections():
    text = (
        encode_loc_tokens([0.0, 0.0, 50.0, 50.0]) + " river ; "
        + encode_loc_tokens([100.0, 100.0, 150.0, 150.0]) + " road"
    )
    decoded = decode_loc_tokens(text, canvas_size=224)
    assert [label for _, label in decoded] == ["river", "road"]


def test_decode_ignores_text_without_enough_tokens():
    assert decode_loc_tokens("no boxes here", canvas_size=224) == []
    assert decode_loc_tokens("<loc0010><loc0020> partial", canvas_size=224) == []


def test_detect_prompt_strips_trailing_periods():
    assert build_detect_prompt(["water body.", "road."]) == "detect water body ; road"


def test_detect_suffix_pairs_boxes_with_labels():
    suffix = build_detect_suffix([[0, 0, 112, 112]], ["lake"], canvas_size=224)
    decoded = decode_loc_tokens(suffix, canvas_size=224)
    assert len(decoded) == 1
    assert decoded[0][1] == "lake"
